...
** Surround (Embrace)
Add/change/delete surrounding pairs. Works with expand-region for semantic unit selection.
#+begin_src emacs-lisp
  (use-package embrace
    :commands (embrace-add embrace-change embrace-delete)
...
#+begin_src emacs-lisp
  (use-package meow
    ...
    ;; Define m prefix keymap for match/surround commands (helix-style)
    (define-prefix-command 'meow-match-keymap)
    (define-key meow-match-keymap (kbd "s") #'embrace-add)
    (define-key meow-match-keymap (kbd "r") #'embrace-change)
    (define-key meow-match-keymap (kbd "d") #'embrace-delete)
    (define-key meow-match-keymap (kbd "m") #'meow-block)

    (defun my/eldoc-box-help-at-point ()
      "Show hover documentation at point using eldoc-box.
Uses eglot's hover if available, otherwise falls back to general eldoc."
      (interactive)
    ...
    (defun my/meow-insert-at-bol ()
      "Move to first non-whitespace character of line and enter insert mode."
      (interactive)
      (back-to-indentation)
      (meow-insert))

    (defun my/meow-append-at-eol ()
    ...
    (defun my/format-region ()
      "Format the selected region using eglot-format or indent-region.
Requires an active selection. Uses eglot-format if the LSP server
supports textDocument/rangeFormatting, otherwise falls back to indent-region."
      (interactive)
      (if (not (region-active-p))
          (message "No selection - select region first")
        (if (and (bound-and-true-p eglot--managed-mode)
                 (eglot-current-server)
    ...
    (defun meow-setup-helix ()
      ...
      (meow-normal-define-key
       ...
       '("i" . meow-insert)
       '("a" . meow-append)
       '("I" . my/meow-insert-at-bol)
       '("A" . my/meow-append-at-eol)
       '("o" . meow-open-below)
       '("O" . meow-open-above)
       ...
       ;; m prefix commands (match/surround)
       '("m" . meow-match-keymap)))
...
** gptel Planning Mode
Planning preset for read-only analysis mode similar to OpenCode's plan mode.
Uses Claude Haiku 4.5 for cost-effective analysis without making changes.
#+begin_src emacs-lisp
  (with-eval-after-load 'gptel
    ;; Add planning directive to gptel-directives
    (add-to-list 'gptel-directives
      '(planning . "You are an AI coding assistant in PLANNING MODE. Your role is to analyze code,
create detailed implementation plans, and provide guidance WITHOUT making any
changes to the codebase.

Core Principles:
- ANALYZE and UNDERSTAND the codebase thoroughly before suggesting changes
- CREATE detailed, step-by-step implementation plans
- IDENTIFY potential issues, edge cases, and tradeoffs
- ASK clarifying questions when requirements are ambiguous
- SUGGEST changes but DO NOT implement them directly
- THINK critically about architecture and design decisions

When the user asks you to implement a feature or fix:
1. First, gather context by reading relevant files and understanding the current
   implementation
2. Analyze the existing patterns, conventions, and architecture
...
4. Present the plan clearly with numbered steps
5. Ask for user confirmation before proceeding to implementation

You have access to read-only tools to inspect the codebase. All modification
tools will require explicit user confirmation. Focus on thorough analysis and
planning rather than rushing to implementation.

Be concise but comprehensive. Provide enough detail for the user to understand
your reasoning and make informed decisions."))

    ;; Create planning preset with Haiku 4.5 and lower temperature
    (gptel-make-preset 'planning
      :description "Planning mode: analyze and plan without making changes (uses Claude Haiku 4)"
      :system 'planning
      :model 'claude-haiku-4-5-20251001
      :temperature 0.1)

    ;; Add keybinding to activate planning mode
    (defun gptel-use-planning-preset ()
      "Switch to planning mode preset."
      (interactive)
      (gptel-use-preset 'planning)
      (message "Switched to planning mode (Claude Haiku 4.5, temp: 0.1)"))

    ;; Bind to C-c g p
    (define-key gptel-mode-map (kbd "C-c g p") #'gptel-use-planning-preset))
#+end_src

** gptel-tools
Comprehensive tool suite for gptel enabling claude-code-ide style capabilities.
Includes tools for: Emacs introspection, Git (via magit), LSP/eglot, compilation,
advanced editing, navigation, environment inspection, and documentation lookup.
#+begin_src emacs-lisp
